...
int main() {
    ...
    // 指定 SAT 编码输出文件的路径
    std::string sat_output_path = "/rshome/daikang.kuang/logicfactory_tool/FastEx/exact_synthesis_DITT/experiment/output.cnf";
    ...
    // 示例 SAT 解
    auto sat_solution = exact::synthesis(&ditt, "kissat", std::cout);

    // 指定解码后的逻辑电路输出文件的路径
    std::string circuit_output_path = "/rshome/daikang.kuang/logicfactory_tool/FastEx/exact_synthesis_DITT/experiment/decoded_circuit.blif";
    ditt.decode(sat_solution.second, circuit_output_path);
